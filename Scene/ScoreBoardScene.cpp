...
void ScoreBoardScene::Initialize() {
    ...
    for (int i = 0; i < PAGE_LINE && i < total_line; i++) {
        AddNewObject(scoreboard[i][0] = new Engine::Label(std::to_string(i + 1) + ". "+ table[i].name, "pirulen.ttf", 28, halfW * 1 / 2 - 100, halfH +  50 * (i - PAGE_LINE / 2), 255, 255, 255, 255, 0, 0.5, 0));
        AddNewObject(scoreboard[i][1] = new Engine::Label(std::to_string(table[i].score), "pirulen.ttf", 28, halfW - 100, halfH +  50 * (i - PAGE_LINE / 2), 255, 255, 255, 255, 0, 0.5, 0));
        AddNewObject(scoreboard[i][2] = new Engine::Label(table[i].time, "pirulen.ttf", 20, halfW * 3 / 2 - 100, halfH +  50 * (i - PAGE_LINE / 2), 255, 255, 255, 255, 0, 0.5, 0));
    }
    current_head_line = 0;
...
void ScoreBoardScene::UpOnClick(int line) {
    ...
    for (int i = 0; i < PAGE_LINE; i++) {
        ...
        scoreboard[i][2]->Text = (i + current_head_line < total_line) ? table[i + current_head_line].time : "" ;
    }
}
